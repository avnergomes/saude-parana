...
- Município: o campo MUNICIPIO do CNPJ é o código TOM da jurisdição fiscal, não IBGE. A conversão usa a tabela oficial `municipios.csv` da Receita (versionada em `data/raw/cnpj/`), validada contra os 399 códigos do `geo_map.json`. No CSV o TOM vem sem zeros à esquerda e no arquivo de estabelecimentos com 4 dígitos; os dois lados são normalizados antes do cruzamento. Se `www.gov.br` recusar o download no runner, vale a cópia versionada, com aviso no log.
- Fonte dos zips: do GitHub Actions o servidor da Receita (Serpro) não aceita conexão; o `connect()` expira em todas as rotas, até na raiz do host, enquanto de um IP no Brasil a mesma chamada responde em 0,3 s (medido em 2026-09-14). No modo `auto`, o job `descobrir` sonda o WebDAV oficial com uma única tentativa de 10 s e, sem resposta, usa o espelho público da Casa dos Dados (`dados-abertos-rf-cnpj.casadosdados.com.br/arquivos/`, autoindex atrás do Cloudflare, sem autenticação). O espelho guarda cada competência numa pasta com a data da publicação (`AAAA-MM-DD`). Três verificações sustentam o uso: a parte 5 de 2026-08 baixada do espelho tem o mesmo sha256 da baixada da Receita; em 7 cópias de 2024-08 a 2026-08, a data de extração no nome do membro do zip (`.D60808.` = 08/08/2026) caiu sempre no mês da pasta; e o `filtrar.py` falha se ano e mês do membro não forem os da competência pedida. O `metadata.fonte` do JSON registra quando o espelho foi usado.
- A competência mais recente é escolhida pela listagem da fonte (PROPFIND na raiz do compartilhamento, só pastas `AAAA-MM`, ignorando o arquivo solto `cnpj.tar.gz`; ou o índice do espelho) e precisa ter as 10 partes de Estabelecimentos; se a última estiver incompleta, usa-se a anterior. Se a competência escolhida já for a publicada em `cnpj_saude.json`, o download é pulado.
...
- Acesso: no WebDAV oficial, HTTP Basic com o token do link público como usuário e senha vazia. O token é o identificador do compartilhamento publicado na página da Receita, o mesmo que um navegador usa; não é credencial nem segredo. O espelho não pede autenticação, e o token não é enviado a ele.
...
| Origem | Dados Abertos do CNPJ da Receita Federal, lidos do WebDAV oficial ou do espelho público da Casa dos Dados (cópia idêntica, conferida pela data de extração de cada zip) |
